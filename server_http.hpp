...
public:
    bool operator()(const std::string& key1, const std::string& key2) const
    {
        if (key1.length() == key2.length())
            return std::equal(key2.begin(), key2.end(), key1.begin(), [&](char a, char b) {
...
namespace SimpleWeb
{
    ...
    private:
        ...
        public:
            ...
            bool operator<(const regex_orderable& rhs) const { return str < rhs.str; }
        };
    ...
    public:
        ...
        virtual void start()
        {
            ...
            if (!acceptor)
                acceptor =
                    std::unique_ptr<asio::ip::tcp::acceptor>(new asio::ip::tcp::acceptor(*io_service));
                  ...
                  const std::function<void(const asio::error_code&)>& callback = nullptr) const
        {
            asio::async_write(
                *response->socket,
                response->streambuf,
                [this, response, callback](const asio::error_code& ec, size_t /*bytes_transferred*/) {
                    if (callback)
                        callback(ec);
                });
        }
    ...
    protected:
        ...
        std::shared_ptr<asio::steady_timer> get_timeout_timer(const std::shared_ptr<socket_type>& socket,
                                                                       long seconds)
        {
            if (seconds == 0)
                return nullptr;
            ...
            timer->expires_from_now(posix_time::seconds(seconds));
                ...
                *socket,
                ...
                [this, socket, request, timer](const asio::error_code& ec, size_t bytes_transferred) {
                    ...
                    if (!ec) {
                        ...
                        //"After a successful async_read_until operation, the streambuf may contain additional data
                        // beyond the delimiter"  The chosen solution is to extract lines from the stream directly when
                        // parsing the header. What is left of the  streambuf (maybe some bytes of the content) is
                        // appended to in the async_read-function below (for retrieving content).
                        ...
                        if (it != request->header.end()) {
                            ...
                            try {
                                ...
                                if (on_error)
                                    on_error(request,
                                             asio::error_code(system::errc::protocol_error,
                            ...
                            if (content_length > num_additional_bytes) {
                                ...
                                asio::async_read(
                                    *socket,
                                    ...
                                    [this, socket, request, timer](const asio::error_code& ec,
                               ...
                               std::shared_ptr<typename ServerBase<socket_type>::Request>)>& resource_function)
        {
            ...
            auto response =
                std::shared_ptr<Response>(new Response(socket), [this, request, timer](Response* response_ptr) {
                    auto response = std::shared_ptr<Response>(response_ptr);
                    this->send(response, [this, response, request, timer](const asio::error_code& ec) {
                        if (timer)
                            timer->cancel();
                        if (!ec) {
            ...
            try {
                ...
                if (on_error)
                    on_error(request,
                             asio::error_code(system::errc::operation_canceled,
                                                       system::generic_category()));
                return;
            }
        }
    ...
    {
    public:
        DEPRECATED Server(unsigned short port,
                          size_t thread_pool_size = 1,
