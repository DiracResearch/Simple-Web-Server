...
{
public:
    bool operator()(const std::string& key1, const std::string& key2) const { return algorithm::iequals(key1, key2); }
};
...
namespace SimpleWeb
{
    ...
    public:
            ...
            const std::map<std::string, std::string>& header = std::map<std::string, std::string>())
        {
            ...
            asio::async_write(*socket,
                              write_buffer,
                              [this, &content, timer](const asio::error_code& ec, size_t /*bytes_transferred*/) {
                                  if (timer)
                                      timer->cancel();
                                  if (!ec) {
                                      if (!content.empty()) {
                                          ...
                                          asio::async_write(
                                              *socket,
                                              ...
                                              [this, timer](const asio::error_code& ec, size_t /*bytes_transferred*/) {
                                                  if (timer)
                                                      timer->cancel();
                                                  if (ec) {
            ...
            const std::map<std::string, std::string>& header = std::map<std::string, std::string>())
        {
            ...
            auto timer = get_timeout_timer();
            asio::async_write(
                *socket, write_buffer, [this, timer](const asio::error_code& ec, size_t /*bytes_transferred*/) {
                    if (timer)
                        timer->cancel();
                    if (ec) {
                        std::lock_guard<std::mutex> lock(socket_mutex);
    ...
    protected:
        ...
        std::shared_ptr<asio::steady_timer> get_timeout_timer(size_t timeout = 0)
        {
            if (timeout == 0)
                ...
                while ((param_end = line.find(':')) != std::string::npos) {
                    ...
                    if ((value_start) < line.size()) {
                        if (line[value_start] == ' ')
                            value_start++;
                        if (value_start < line.size())
                            response->header.insert(std::make_pair(
                                line.substr(0, param_end), line.substr(value_start, line.size() - value_start - 1)));
                    }
        ...
        std::shared_ptr<Response> request_read()
        {
            ...
            asio::async_read_until(
                *socket,
                ...
                "\r\n\r\n",
                [this, &response, &chunked_streambuf, timer](const asio::error_code& ec, size_t bytes_transferred) {
                    if (timer)
                        timer->cancel();
                    if (!ec) {
                        ...
                        } else if ((header_it = response->header.find("Transfer-Encoding")) != response->header.end() &&
                                   header_it->second == "chunked") {
                            request_read_chunked(response, chunked_streambuf);
                        } else if (response->http_version < "1.1" ||
                                   ((header_it = response->header.find("Connection")) != response->header.end() &&
        ...
        void request_read_chunked(const std::shared_ptr<Response>& response, asio::streambuf& streambuf)
        {
            ...
            asio::async_read_until(
                *socket,
                response->content_buffer,
                "\r\n",
                [this, &response, &streambuf, timer](const asio::error_code& ec, size_t bytes_transferred) {
                    if (timer)
                        timer->cancel();
                    if (!ec) {
                        std::string line;
                        ...
                        auto num_additional_bytes =
                            static_cast<std::streamsize>(response->content_buffer.size() - bytes_transferred);
                        ...
                        if ((2 + length) > num_additional_bytes) {
                            ...
                            asio::async_read(
                                *socket,
                                ...
                                [this, post_process, timer](const asio::error_code& ec, size_t /*bytes_transferred*/) {
                                    if (timer)
                                        timer->cancel();
                                    if (!ec) {
                                        post_process();
        ...
        void connect()
        {
            if (!socket || !socket->is_open()) {
                ...
                if (config.proxy_server.empty())
                    ...
                    query = std::unique_ptr<asio::ip::tcp::resolver::query>(new asio::ip::tcp::resolver::query(
                        proxy_host_port.first, std::to_string(proxy_host_port.second)));
                }
                resolver.async_resolve(
                    *query, [this](const asio::error_code& ec, asio::ip::tcp::resolver::iterator it) {
                        if (!ec) {
                            {
                                std::lock_guard<std::mutex> lock(socket_mutex);
                            ...
                            asio::async_connect(
                                *socket,
                                it,
                                [this, timer](const asio::error_code& ec, asio::ip::tcp::resolver::iterator /*it*/) {
                                    if (timer)
                                        timer->cancel();
                                    if (!ec) {
